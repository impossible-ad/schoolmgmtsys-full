...
export const addTeacher = async (req, res, next) => {
  ...
  try {
    ...
    if (givenemail.length > 0) {
      //0 denotes no of id present in database array
...
export const getAllTeacher = async (req, res, next) => {
  try {
    ...
    const [result] = await db.execute(
      "SELECT * FROM TEACHER ORDER BY id DESC limit ? OFFSET ?",
      [limit, offset]
    );
    ...
    res.status(200).json({
      ...
      remainingteachers: total - (offset + limit),
    });
  } catch (error) {
    next(error);
  }
